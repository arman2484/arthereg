...
@section('content')
    <h4 class="py-3 mb-4">
        <span class="text-muted fw-light">Tax /</span> Edit Tax
    </h4>

    <form action="{{ url('app/ecommerce/tax/update/' . $taxRate->id) }}" method="POST"
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="title" name="name"
                                    aria-label="category title" placeholder="Enter title" value="{{ $taxRate->name }}">
                                    ...
                                    <select id="zone_id" name="zone_id" class="select2 form-select"
                                        ...
                                        @foreach ($data as $value)
                                            <option value="{{ $value->id }}"
                                                {{ $value->id == $taxRate->zone_id ? 'selected' : '' }}>
                                                {{ $value->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <span id="country-error" class="error">{{ $errors->first('zone_id') }}</span>
                                </div>
