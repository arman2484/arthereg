...
$(function () {
  ...
  var dt_order_table = $('.datatables-shipping'),
    statusObj = {
  ...
  if (dt_order_table.length) {
    dt_products = dt_order_table.DataTable({
      ...
      serverSide: true,
      pageLength: 10,
      ajax: '/app/ecommerce/shipping/list/data',
      columns: [
      ...
      columnDefs: [
        { visible: false, targets: [0] },
        {
          ...
          render: function (data, type, full, meta) {
            return '<span class="text-nowrap">' + full['delivery_title'] + '</span>';
          }
        },
        {
          targets: 2,
          render: function (data, type, full, meta) {
            return '<span class="text-nowrap">' + full['price'] + '</span>';
          }
        },
...

        {
          ...
          orderable: false,
          render: function (data, type, full, meta) {
            var editUrl = baseUrl + 'app/ecommerce/shipping/edit/' + full.id;
            return (
              '<div class="d-inline-block text-nowrap">' +
              '<a href="' +editUrl +'" class="btn btn-sm btn-icon"><i class="bx bx-edit"></i></a>' +
              '<a href="javascript:0;" class="btn btn-sm btn-icon delete-record" id="shipping-deleted-' +
              full.id +
      ...
      buttons: [
        {
          text: '<i class="bx bx-plus me-0 me-sm-1"></i>Add Shipping',
          className: 'add-new btn btn-primary ms-2',
          action: function () {
            window.location.pathname = 'app/ecommerce/shipping/add';
          }
        }
      ],
...
function changeStatus(id, status) {
  Swal.fire({
    ...
    if (result.value) {
      $.ajax({
        url: baseUrl + 'app/ecommerce/shipping/status/' + id,
        data: {
          status: status
        },
        type: 'get',
...
function deleteShipping(id) {
  Swal.fire({
    ...
    icon: 'warning',
    showCancelButton: true,
    confirmButtonText: 'Yes, Delete Shipping!',
    customClass: {
      confirmButton: 'btn btn-primary me-2',
      cancelButton: 'btn btn-label-secondary'
    },
    ...
    if (result.value) {
      $.ajax({
        url: baseUrl + 'app/ecommerce/shipping/delete/' + id,
        type: 'POST',
        headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        success: function (data) {
          var parentTr = $('#shipping-deleted-' + data.id).closest('tr');
          parentTr.remove();
          Swal.fire({
            icon: 'success',
            title: 'Deleted!',
