...
$(function () {
  ...
  var dt_order_table = $('.datatables-tax-rate'),
    statusObj = {
  ...
  if (dt_order_table.length) {
    dt_products = dt_order_table.DataTable({
      ...
      serverSide: true,
      pageLength: 10,
      ajax: '/app/ecommerce/tax/list/data',
      columns: [
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            if (full['type'] == 0) {
              return '<span class="text-nowrap">' + 'Percentage' + '</span>';
            } else {
              return '<span class="text-nowrap">' + 'Fix amount' + '</span>';
            }
          }
        },
...

        {
          ...
          orderable: false,
          render: function (data, type, full, meta) {
            var editUrl = baseUrl + 'app/ecommerce/tax/edit/' + full.id;
            return (
              '<div class="d-inline-block text-nowrap">' +
              '<a href="' +
              editUrl +
              '" class="btn btn-sm btn-icon"><i class="bx bx-edit"></i></a>' +
              '<a href="javascript:0;" class="btn btn-sm btn-icon delete-record" id="tax-rate-deleted-' +
              full.id +
      ...
      buttons: [
        {
          ...
          action: function () {
            window.location.pathname = 'app/ecommerce/tax/rate/add';
          }
        }
      ],
      responsive: {
...
function changeStatus(id, status) {
  Swal.fire({
    ...
    if (result.value) {
      $.ajax({
        url: baseUrl + 'app/ecommerce/tax/rate/status/' + id,
        data: {
          status: status
        },
        type: 'get',
...
function deleteTaxRate(id) {
  Swal.fire({
    ...
    if (result.value) {
      $.ajax({
        url: baseUrl + 'app/ecommerce/tax/delete/' + id,
        type: 'POST',
        headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
