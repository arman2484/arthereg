...
$(function () {
  ...
  var dt_order_table = $('.datatables-variant'),
    statusObj = {
  ...
  if (dt_order_table.length) {
    dt_products = dt_order_table.DataTable({
      ...
      serverSide: true,
      pageLength: 10,
      ajax: '/app/ecommerce/variant/list/data',
      columns: [
      ...
      columnDefs: [
...

        {
          ...
          orderable: false,
          render: function (data, type, full, meta) {
            var editUrl = baseUrl + 'app/ecommerce/variant/edit/' + full.id;
            return (
              '<div class="d-inline-block text-nowrap">' +
              '<a href="' +
              editUrl +
              '" class="btn btn-sm btn-icon"><i class="bx bx-edit"></i></a>' +
              '<a href="javascript:0;" class="btn btn-sm btn-icon delete-record" id="variant-deleted-' +
              full.id +
      ...
      buttons: [
        {
          text: '<i class="bx bx-plus me-0 me-sm-1"></i>Add Variant',
          className: 'add-new btn btn-primary ms-2',
          action: function () {
            window.location.pathname = 'app/ecommerce/variant/add';
          }
        }
      ],
      responsive: {
...
function changeStatus(id, status) {
  Swal.fire({
    ...
    if (result.value) {
      $.ajax({
        url: baseUrl + 'app/ecommerce/variant/status/' + id,
        data: {
          status: status
        },
        type: 'get',
...
function deleteVariant(id) {
  Swal.fire({
    ...
    if (result.value) {
      $.ajax({
        url: baseUrl + 'app/ecommerce/variant/delete/' + id,
        type: 'POST',
        headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        success: function (data) {
          var parentTr = $('#variant-deleted-' + data.id).closest('tr');
          parentTr.remove();
          Swal.fire({
            icon: 'success',
            title: 'Deleted!',
