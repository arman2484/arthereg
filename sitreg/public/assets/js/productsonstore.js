$(document).ready(function () {
  ...
  window.getProductonStoreList = function () {
    $.ajax({
      type: 'POST',
      ...
      success: function (response) {
        ...
        if (response && response.products && response.products.length > 0) {
          ...
          displayProductsonStore(response.products);
        } else {
  ...
  getProductonStoreList();

  // Function to display product list
  window.displayProductsonStore = function (products) {
    var productContent = $('#getproductonstoredatacapture');
  ...
  function displayNoProductsFound() {
    const container = document.getElementById('getproductonstoredatacapture');
