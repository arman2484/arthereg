...
(function () {
  ...
  if (
    typeof activityAreaChartEl !== undefined &&
    activityAreaChartEl !== null
  ) {
    const activityAreaChart = new ApexCharts(
      activityAreaChartEl,
      activityAreaChartConfig
    );
  ...
  if (typeof profitBarChartEl !== undefined && profitBarChartEl !== null) {
    const profitBarChart = new ApexCharts(
      profitBarChartEl,
      profitBarChartConfig
    );
  ...
  const totalIncomeEl = document.querySelector("#totalIncomeChart"),
    totalIncomeConfig = {
      ...
      yaxis: {
        ...
        // min: 0, // Set the minimum value to 1000
        // max: 0, // Set the maximum value to 10000
        tickAmount: 5,
      },
    };
  ...
  function fetchDataAndUpdateChart() {
    fetch("/app/ecommerce/dashboard/chart/data")
      .then((response) => response.json())
      .then((data) => {
        // const minDataValue = Math.min(...data);
        // const maxDataValue = Math.max(...data);
        // console.log(maxDataValue);
        // // Update the yaxis in totalIncomeConfig
        // totalIncomeConfig.yaxis.min = minDataValue - 1000; // Set a buffer for the minimum value
        // totalIncomeConfig.yaxis.max = maxDataValue + 2000; // Set a buffer for the maximum value
        totalIncomeConfig.series[0].data = data;

        // Update the chart data with the retrieved data

        // Create and render the chart with the updated data and yaxis settings
        const totalIncome = new ApexCharts(totalIncomeEl, totalIncomeConfig);
        totalIncome.render();
      })
      .catch((error) => {
  ...
  if (
    typeof conversionLineChartEl !== undefined &&
    conversionLineChartEl !== null
  ) {
    const conversionLineChart = new ApexCharts(
      conversionLineChartEl,
      conversionLineChartConfig
    );
  ...
  if (typeof expensesBarChartEl !== undefined && expensesBarChartEl !== null) {
    const expensesBarChart = new ApexCharts(
      expensesBarChartEl,
      expensesBarChartConfig
    );
  ...
  if (
    typeof totalBalanceChartEl !== undefined &&
    totalBalanceChartEl !== null
  ) {
