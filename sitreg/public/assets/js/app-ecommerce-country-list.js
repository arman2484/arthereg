...
$(function () {
  ...
  if (dt_order_table.length) {
    dt_products = dt_order_table.DataTable({
      ...
      serverSide: true,
      pageLength: 10,
      ajax: '/app/ecommerce/country/list/data',
      columns: [
      ...
      columnDefs: [
        ...
        },
        {
          targets: 2,
          render: function (data, type, full, meta) {
            return '<span class="text-nowrap">' + full['code'] + '</span>';
          }
        },
...

        {
          ...
          orderable: false,
          render: function (data, type, full, meta) {
            var editUrl = baseUrl + 'app/ecommerce/country/edit/' + full.id;
            return (
              '<div class="d-inline-block text-nowrap">' +
              '<a href="' +
              editUrl +
              '" class="btn btn-sm btn-icon"><i class="bx bx-edit"></i></a>' +
              '<a href="javascript:0;" class="btn btn-sm btn-icon delete-record" id="country-deleted-' +
              full.id +
      ...
      buttons: [
        {
          text: '<i class="bx bx-plus me-0 me-sm-1"></i>Add Country',
          className: 'add-new btn btn-primary ms-2',
          action: function () {
            window.location.pathname = 'app/ecommerce/country/add';
          }
        }
      ],
      responsive: {
...
function changeStatus(id, status) {
  Swal.fire({
    ...
    if (result.value) {
      $.ajax({
        url: baseUrl + 'app/ecommerce/country/status/' + id,
        data: {
          status: status
        },
        type: 'get',
...
function deleteCountry(id) {
  Swal.fire({
    title: 'Are you sure?',
    text: "You won't be able to revert Country!",
    icon: 'warning',
    showCancelButton: true,
    ...
    if (result.value) {
      $.ajax({
        url: baseUrl + 'app/ecommerce/country/delete/' + id,
        type: 'POST',
        headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        success: function (data) {
          var parentTr = $('#country-deleted-' + data.id).closest('tr');
          parentTr.remove();
          Swal.fire({
            icon: 'success',
            title: 'Deleted!',
